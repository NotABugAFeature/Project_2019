...
//    return m;
//}
//int execute_query_parallel(query *q, table_index* index, bool *sorting, uint32_t* bool_counter, uint32_t *predicate_index, middleman** middle, uint32_t*** joined_tables)
int execute_query_parallel(job_query_parameters* p)
{
    ...
    while(p->pred_index<p->query->number_of_predicates)
    {
                 ...
                 (((predicate_join *) p->query->predicates[p->pred_index].p)->r.table_id!=((predicate_join *) p->query->predicates[p->pred_index].p)->s.table_id)))
        {
            ...
            while(list_temp_r!=NULL)
            {
                if(self_join_middle_bucket(join, table_r, table_s, &(list_temp_r->bucket), &(list_temp_s->bucket), result_R, result_S, index_list, &r_index, &s_index, &counter))
                {
                    fprintf(stderr, "execute_query: Error in self_join_middle_bucket\n");
                    return -10;
                }
...
//        if(m->tables[p.column_to_project.table_id].list==NULL||m->tables[p.column_to_project.table_id].list->number_of_records==0)
//        {
//            fprintf(stderr, "\e[1;33mNULL \e[0m");
//            continue;
//        }
